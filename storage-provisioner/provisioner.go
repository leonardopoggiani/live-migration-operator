...
func NewCheckpointProvisioner(ctx context.Context, cl client.Client, storageNamespace string) (controller.Provisioner, error) {
	...
	ns := &corev1.Namespace{}
	err := cl.Get(ctx, client.ObjectKey{Name: storageNamespace}, ns)
	if err != nil {
		if errors.IsNotFound(err) {
			...
			klog.ErrorS(err, "Failed to get the storage namespace", "storageNamespace", storageNamespace)
		}
	} else {
		klog.InfoS("Storage namespace already exists", "storageNamespace", storageNamespace)
	}

	return &CheckpointProvisioner{
		client:                  cl,
		virtualStorageClassName: "checkpoint-storage",
...
func SetupWithManager(mgr ctrl.Manager) error {
	klog.Infof("Setting up checkpoint provisioner controller")
